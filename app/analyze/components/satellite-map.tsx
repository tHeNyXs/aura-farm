...
export default function SatelliteMap({
  ...
  return (
    <div className="w-full h-full relative">
      ...
      {zoningCropId && (
        <div
          className="absolute right-4 top-4 z-[1000] w-52 rounded-xl p-3.5 text-xs shadow-2xl pointer-events-none"
          style={{
            background: "#091A12",
        ...
        >
          <div className="mb-2 font-bold font-heading text-sm" style={{ color: "#2CC9A0" }}>ระดับความเหมาะสม LDD</div>
          {Object.entries(ZONING_STYLES).map(([level, style]) => (
            <div key={level} className="mb-1.5 flex items-center gap-2 last:mb-0">
              <span className="h-3 w-3 rounded-xs shrink-0" style={{ backgroundColor: style.fillColor }} />
              <span className="text-gray-200"><b>{level}</b> — {style.label}</span>
            </div>
          ))}
          {zoningStatus && <div className="mt-2 border-t border-emerald-900/60 pt-2 text-[11px] text-emerald-300 font-mono">{zoningStatus}</div>}
        </div>
      )}

      {/* Coordinate & Scale HUD Overlay at Bottom-Left */}
      <div
        className="absolute bottom-6 left-6 z-[1000] rounded-xl px-3.5 py-2 flex items-center gap-3 font-mono text-[11px] select-none pointer-events-none shadow-2xl"
        style={{
          background: "#091A12",
          border: "1px solid rgba(44, 201, 160, 0.35)",
          color: "#E8F5EF",
          boxShadow: "0 8px 24px rgba(0,0,0,0.6)",
        }}
      >
        <span ref={gridCodeRef} className="font-bold" style={{ color: "#2CC9A0" }}>
          GRID S-29-W
        </span>
        <span className="w-px h-3 bg-emerald-700/60" />
        <span ref={scaleRef} className="text-gray-300">SCALE 1:5,000</span>
        <span className="w-px h-3 bg-emerald-700/60" />
        <span ref={centerCoordsRef} className="text-gray-300">LAT: 13.7500° N, LON: 100.5000° E</span>
      </div>
    </div>
  );
}
